...
export default function ScholarshipCard({ scholarship, onViewDetails, onAddDocuments }) {
  ...
  return (
    <div className="rounded-2xl border border-line-soft bg-white p-5 transition-shadow hover:shadow-md hover:shadow-ink/5">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-[11px] font-semibold uppercase tracking-wide text-ink-faint">
            {s.typeLabel}
          </p>
          <h3 className="mt-0.5 font-display text-[17px] font-bold leading-snug text-ink">{s.name}</h3>
          <p className="mt-0.5 text-[13px] text-ink-soft">{s.provider}</p>
        </div>
      ...
      {s.status === 'not-eligible' && s.notEligibleReason && (
        <p className="mt-3 text-sm leading-relaxed text-ink-soft">{s.notEligibleReason}</p>
      )}
      ...
      {s.status === 'eligible' && (
        <p className="mt-3 text-sm leading-relaxed text-ink-soft">{s.whyMatch}</p>
      )}
