...
import OptionalDocumentsUpload from "../components/OptionalDocumentsUpload";
import StudentDetailsFields from "../components/StudentDetailsFields";
import ExtractionSummary from "../components/ExtractionSummary";
import DocumentsExtractionSummary from "../components/DocumentsExtractionSummary";
...
export default function ScholarshipMatchTestPage() {
    ...
    const [optionalDocs, setOptionalDocs] = useState(EMPTY_DOCUMENTS);
    const [fields, setFields] = useState(EMPTY_FIELDS);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [response, setResponse] = useState(null);

    function handleOptionalDocChange(key, selectedFile) {
        setOptionalDocs((prev) => ({ ...prev, [key]: selectedFile }));
    }
    ...
    async function handleSubmit(e) {
        ...
        try {
            const data = await extractAndMatch(file, fields, optionalDocs);
            setResponse(data);
        } catch (err) {
    ...
    return (
        <div className="mx-auto max-w-2xl p-6">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <OptionalDocumentsUpload
                    files={optionalDocs}
                    onFileChange={handleOptionalDocChange}
                />
            ...
            {response && (
                <div className="mt-6 flex flex-col gap-6">
                    ...
                    <DocumentsExtractionSummary
                        documentsExtracted={response.documents_extracted}
                    />
