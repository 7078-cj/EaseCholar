...
export default function AIMatching({ onComplete, onError }) {
  ...
  const documents = useDocumentsStore((s) => ({ reportCard: s.reportCard, optional: s.optional }))
  ...
  if (apiStatus === 'error') {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-bg px-6 text-center">
        ...
        <h1 className="font-display text-xl font-bold text-ink">We couldn't finish matching</h1>
        <p className="mt-2 max-w-sm text-sm leading-relaxed text-ink-soft">{apiError}</p>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg px-6 py-10">
      <div className="mb-12 flex items-center gap-2.5">
        ...
        <span className="font-display text-xl font-bold text-ink">EaseKolar</span>
      </div>
      ...
      <div className="relative mb-10 h-[100px] w-[100px]">
        ...
        >
          {done ? (
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="animate-checkTick">
      ...
      <p className="mb-10 mt-2 text-center text-[15px] text-ink-soft">
        {done
          ...
          : 'Our AI is analyzing your profile against every scholarship in our database.'}
      </p>
      ...
      <div className="flex w-full max-w-[400px] flex-col gap-3.5 rounded-2xl border border-line-soft bg-white p-6">
        {STEPS.map((step, i) => {
          ...
          return (
            <div key={step.label} className={`flex items-center gap-3.5 transition-opacity duration-400 ${isChecked ? 'opacity-100' : 'opacity-35'}`}>
              <div
                className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-colors ${
                  isChecked ? (isLast && !done ? 'bg-warn' : 'bg-success') : 'bg-line-soft'
                }`}
              >
                {isChecked && (
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="animate-checkTick">
                    <path d="M20 6L9 17l-5-5" />
                  </svg>
                )}
              </div>
              <span className={`text-sm ${isLast ? 'font-display font-bold' : 'font-medium'} ${isChecked ? 'text-ink' : 'text-ink-faint'}`}>
                {step.label}
              </span>
            </div>
          )
